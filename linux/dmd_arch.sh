...
# check if destination package file already exist
if test -f $DESTDIR"/"$ARCHFILE ;then
	ferror "\"$DESTDIR/$ARCHFILE\" already exist" "Exiting..."
fi
...
if test -f $DESTDIR"/"d-completion.sh ;then
	...
	cp $DESTDIR"/"d-completion.sh $BASEDIR"/root/etc/bash_completion.d/dmd"
fi
